(function () {
    ...
    // --- localStorage persistence (primary calculator only) ---
    var STORAGE_KEY = 'calculator.primary';
    ...
    function loadPersistedState() {
        try {
            ...
            return {
                radix: [16, 10, 8, 2].indexOf(data.radix) !== -1 ? data.radix : 10,
                currentVal: BigInt(data.currentVal || '0'),
                memory: BigInt(data.memory || '0')
            };
        } catch (e) {
            return null;
        }
    ...
    function savePersistedState(radix, currentVal, memory) {
        try {
    ...
    function createCalc(container, closable, persist) {
        var id = instanceId++;
        var radixName = 'calcRadix_' + id;
        var saved = persist ? loadPersistedState() : null;
        ...
        [['16', 'Hex'], ['10', 'Dec'], ['8', 'Oct'], ['2', 'Bin']].forEach(function (r) {
            ...
            inp.name = radixName;
            inp.value = r[0];
            if (parseInt(r[0], 10) === (saved ? saved.radix : 10)) inp.checked = true;
            lbl.appendChild(inp);
        ...
        // --- Calculator state ---
        var radix = saved ? saved.radix : 10;
        var currentVal = saved ? saved.currentVal : BigInt(0);
        var pendingOp = null;
        var pendingVal = null;
        var freshEntry = true;
        var memory = saved ? saved.memory : BigInt(0);

        function persist_() {
            if (persist) savePersistedState(radix, currentVal, memory);
        }
    ...
    createCalc(container, false, true);
    ...
    addBtn.addEventListener('click', function () {
        var newCalc = createCalc(container, true, false);
        newCalc.focus();
    });
