...
information is useful in context of reverse engineering. I tried my best not to
confuse the reader with too much of jargon, and help any newbie to get up to the
speed.

[GNU compiler collection](https://en.wikipedia.org/wiki/GNU_Compiler_Collection) provides an
excellent set of tools for dissecting the compilation process and to
understand the working of bits and bytes in the final executable. For this
tutorial I am using the following tools and considers C language to
...
for Linux environment are also available for Windows(thanks to
Many people don't know that the powerful programmer tools that are available
Cygwin/Mingw). Dev-Cpp is an IDE which consists of entire GNU compiler
collection(GCC). This IDE uses gcc.exe, g++.exe for compiling C, C++
programs and gdb.exe for debugging. Also, The Dev-Cpp's
installation(c:\dev-cpp\bin) directory consists of all the GNU compiler
tools. To start this tutorial set the PATH variable to Dev-Cpp's bin
directory as we will be working completely from the command prompt and not
going to use the IDE.
...
SS:[EBP-4], 0} Compares the value at SS:[EBP-4](reg) with zero, So this is
...
- [Code to browse PE File](https://github.com/vineelkovvuri/ExeFileDumper/blob/master/ExeDumper.c)
- [Linkers and Loaders by John R Levine](http://amzn.com/1558604960)
- [Engineering A Compiler by Keith Cooper, Linda Torczon](http://amzn.com/012088478X)
- [Awesome Reverse Engineering Playground/Community](http://crackmes.de)
