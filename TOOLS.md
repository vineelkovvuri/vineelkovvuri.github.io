...
The Hugo theme (in the root `theme/` folder) provides page margins via:
...
### Tools needing extra CSS or scripts

Add a scoped `<style>` block and/or an `index.js` right after the front matter — **no** `<html>`, `<head>`, or `<body>` tags. The theme already provides the page shell and loads the site CSS (`/css/style.css`, `/css/fonts.css`, `/css/copy-button.css`, `/css/forms.css`) from the root, so **do not** copy CSS into a local `css/` folder.
...
- The help text can go inside a placeholder div shown before the user loads data (see Hex Editor's `hex-placeholder`).
...
### Large tool (separate JS + assets)

```text
content/tools/my-tool/
  index.html      # Front matter + <style> + HTML + <script src="./index.js">
  index.js        # Application JavaScript
  <assets>/       # Only genuinely tool-specific assets (e.g. monaco/ for Monaco editor)
```

Example: PE Editor, Hex Editor, UEFI Log Explorer (`monaco/`)
